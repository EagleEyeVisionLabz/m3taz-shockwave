...
allowed-tools:
  - Bash(firecrawl *)
required-secrets: FIRECRAWL_API_KEY
---
...
`firecrawl` is **bundled with Shockwave** — already installed and on PATH. Do NOT
run `npm install`, `firecrawl init`, or `firecrawl login`. Authentication uses the
**agent secret** `FIRECRAWL_API_KEY`: read it with `get_agent_secret` and pass it
inline as an env var, e.g. `FIRECRAWL_API_KEY="<key>" firecrawl ...`. See
[rules/install.md](rules/install.md). Check status with
